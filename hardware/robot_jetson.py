...
class DrivebaseJetson(DrivebaseBase):
    def __init__(self):
        self.IN1 = 22
        self.IN2 = 24
        self.IN3 = 17
...
class RockerBogieJetson(RockerBogieBase):
    def __init__(self):
        i2c = busio.I2C(board.SCL, board.SDA)
        self.pca = PCA9685(i2c)
        self.pca.frequency = 50
        self.servos = [adafruit_servo.Servo(self.pca.channels[i]) for i in range(4)]

    def setPositions(self, positions):
        for index, pos in enumerate(positions):
            self.servos[index].angle = pos


def make_rover() -> Rover:
    return Rover(DrivebaseJetson(), RockerBogieJetson())
